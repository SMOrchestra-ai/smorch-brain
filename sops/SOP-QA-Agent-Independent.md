# SOP: Independent QA Agent (smo-eo-qa)
...
|-----------|-------|------|
| smo-eo-qa server | 84.247.172.113 (Tailscale 100.99.145.22) | Runs the QA agent |
| QA runner script | `/opt/qa-runner.sh` | Clones PR branch, runs `/score-project`, posts comment |
| n8n workflow | flow.smorchestra.ai `qa-agent-trigger` | GitHub webhook → SSH to smo-eo-qa → run script |
| GitHub branch protection | `dev` branch, each repo | Requires `qa-score` status check ≥ 90 |
| Skills on smo-eo-qa | `/root/.claude/skills/` (82 skills) | Same as dev machines — gstack, superpowers, smorch-dev-scoring |
...
- **Different machine:** smo-eo-qa has no access to developer's local files or session history
...
Independent score on smo-eo-qa: ~75 (honest)

Gap = what's actually missing for production quality.

## Operational Rules

1. **smo-eo-qa NEVER writes code.** QA runner is read-only on repos. Posts comments only.
2. **smo-eo-qa NEVER pushes.** Git remote is read-only.
...
- [x] smo-eo-qa provisioned: Ubuntu 24.04, Node 22, npm 10.9, bun 1.3.12
...
- [x] Drift detection extended to include smo-eo-qa
...
- [ ] Add SSH credential to smo-eo-qa in n8n credentials
...
| Scenario | Action |
|----------|--------|
| QA agent fails to run | Telegram alert + check smo-eo-qa health + check n8n logs |
| QA score 0 on legitimate PR | Re-run manually via `ssh smo-eo-qa; bash /opt/qa-runner.sh ...` |
| Drift detected on smo-eo-qa | Fix via `smorch-brain/canonical/*-smo-eo-qa.*` baseline |
