...
- **name**: Skill identifier. **MUST contain only lowercase letters, numbers, and hyphens** (e.g., `gtm-fitness-scoring`, `my-cool-skill`). No uppercase letters, no spaces, no underscores, no special characters. This is a hard technical requirement — skills with invalid names will fail to register. Always validate the name against the regex `^[a-z0-9]+(-[a-z0-9]+)*$` before writing the SKILL.md frontmatter. If the user suggests a name with uppercase or spaces, silently convert it (e.g., "GTM Fitness Scoring Engine" → `gtm-fitness-scoring-engine`).
...
**NAMING RULE (non-negotiable):** Both the directory name AND the `name` field in YAML frontmatter must use only lowercase letters, numbers, and hyphens. No uppercase, no spaces, no underscores. The directory name and `name` field should match. Example: directory `gtm-fitness-scoring/` with `name: gtm-fitness-scoring`. Invalid names break skill registration silently — the skill just won't appear.
