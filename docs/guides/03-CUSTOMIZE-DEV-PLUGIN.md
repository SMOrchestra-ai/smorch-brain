...
1. **Pre-commit hook** validates `.smorch/project.json` against schema. `dev_plugin` field required for any repo with `status-production` or `status-beta`.
2. **`/smo-*` commands** check `dev_plugin` field on invocation — if value is not `smorch-dev`, they refuse and print: "This project uses {plugin}. Run `/{eo|custom}-{command}` instead."
3. **CLAUDE.md line 1** must state which plugin is active (copy-paste format below).
4. **`/smo-drift --desktop`** flags any local clone whose `~/.claude/plugins/` doesn't include the plugin declared in `.smorch/project.json`.
